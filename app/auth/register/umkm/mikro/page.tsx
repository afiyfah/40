...
import {
  ...
  ErrorBox, UploadZone, KTPZone, BankSection, MapPicker, SuccessScreen, IllustrMikro,
...
export default function RegisterMikroPage() {
  ...
  const handleNext = async () => {
    ...
    const data: RegisterUMKMData = {
      role: "umkm_mikro", email, password,
  ...
  if (step === 5) return <SuccessScreen bg="#3b5998" onBack={() => router.push("/auth/login?role=penjual")} />;
  ...
  return (
    <SplitLayout
      ...
      leftIllustration={<IllustrMikro />}
      leftFooter={
        <p style={{ fontSize: 13, color: "#555" }}>
          Sudah mempunyai akun?{" "}
          <Link href="/auth/login?role=penjual" style={{ color: "#3b5998", fontWeight: 700, textDecoration: "underline" }}>Masuk</Link>
        </p>
      }
      rightBg="blue"
    >
      ...
      {step === 4 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.2)", margin: "16px 0 20px" }} />
          <NavRow onBack={() => { setStep(3); setError(""); }} onNext={handleNext} loading={loading} isLast />
        </div>
      )}
    </SplitLayout>
  );
